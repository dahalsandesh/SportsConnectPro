...
import { useGetVenueApplicationByIdQuery } from "@/redux/api/venue-applications/venueApplicationsApi"
...
export default function VenueApplicationDetailClient({ id }: VenueApplicationDetailClientProps) {
  const { toast } = useToast()
  const { data: application, error, isLoading } = useGetVenueApplicationByIdQuery(id)
  ...
  if (error || !application) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>Failed to load venue application details. Please try again later.</AlertDescription>
      </Alert>
    )
  }
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Application submitted on {format(new Date(application.CreatedAt), "MMMM dd, yyyy")}
                </CardDescription>
              </div>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground">Venue Information</h3>
                  ...
                  <div>
                    ...
                    <dl className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <dd>{format(new Date(application.CreatedAt), "MMMM dd, yyyy")}</dd>
                      </div>
                      <div className="flex justify-between">
                        <dt className="font-medium">Reviewed On:</dt>
                        <dd>
                          {application.reviewed_at
                            ? format(new Date(application.reviewed_at), "MMMM dd, yyyy")
                            : "Not reviewed yet"}
                        </dd>
                      </div>
                      <div className="flex justify-between">
              ...
              {application.Status === "pending" && (
                <div className="space-y-4 pt-4">
                  <h3 className="text-sm font-medium text-muted-foreground">Review Application</h3>
                  <Separator className="my-2" />
        ...
        <TabsContent value="documents" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Review the documents submitted with this application</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  {application.document.map((doc, index) => (
                    <Card key={index}>
                      <CardHeader className="pb-2">
                        <CardTitle className="text-base">{doc.docType}</CardTitle>
                      </CardHeader>
                      <CardContent className="pb-2">
                        <div className="flex items-center">
                          <FileText className="h-8 w-8 text-muted-foreground mr-2" />
                          <span className="text-sm truncate">{doc.file.split("/").pop()}</span>
                        </div>
                      </CardContent>
                      <CardFooter>
