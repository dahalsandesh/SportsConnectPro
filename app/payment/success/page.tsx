...
export default function PaymentSuccessPage() {
    ...
    const verifyPayment = async () => {
      try {
        ...
        const paymentInfo = {
          ...
          status,
          purchase_order_id:
            purchase_order_id || searchParams.get("purchaseOrderId"),
          courtId: courtId || "",
          userId,
        };
        ...
        if (paymentStatus === "success" || paymentStatus === "pending") {
          try {
            ...
            const response = await fetch(
              process.env.NEXT_PUBLIC_API_URL + "web/api/v1/user/VerifyPayment",
              {
                method: "POST",
                headers: {
  ...
  const determineStatus = (status?: string | null): PaymentStatus => {
    ...
    if (["pending", "initiated", "processing"].includes(normalizedStatus))
      return "pending";
    if (["cancelled", "canceled", "user_cancelled"].includes(normalizedStatus))
      return "cancelled";
    if (normalizedStatus === "expired") return "expired";
    if (["failed", "rejected", "error"].includes(normalizedStatus))
      return "failed";
    ...
    if (status === "pending") {
      return (
        <div className="flex flex-col items-center justify-center space-y-6 text-center">
          ...
          <p className="text-muted-foreground max-w-md">
            Your payment is being processed. This may take a few minutes. Our
            team will contact you shortly once the payment is confirmed.
          </p>
